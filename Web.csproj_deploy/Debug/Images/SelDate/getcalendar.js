    ...
    var imgDir = "../Images/seldate/images/"
    ...
    */
    ...
    function init(){
        if (!ns4)
        {
            ...
            if (showToday==1)
            {
                 document.getElementById("lblToday").innerHTML = todayString + " <a onmousemove='window.status=\""+gotoString+"\"' onmouseout='window.status=\"\"' title='"+gotoString+"' style='"+styleAnchor+"' href='javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();'>星期"+dayName[(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)]+"  " + yearNow +  "年" + monthName2[monthNow].substring(0,3) + "月" + dateNow + "日</a>   <a onmousemove='window.status=\"清除日期选择\"' onmouseout='window.status=\"\"' title='清除日期选择' style='"+styleAnchor+"' href='javascript:clearCalendar()'>清除</a>"
            }
    ...
    function popUpCalendar(ctl, ctl2, format) {
        ...
        if (bPageLoaded)
        {
            if ( crossobj.visibility == "hidden" ) {
             ...
             tokensChanged = 0
             var isCnDate=ctl2.value.indexOf("年") != -1;
             if ( formatChar != "" || isCnDate)
             {
              // use user's date
              if(isCnDate)
              {
                  aFormat = "yyyy-mm-dd".split("-")
                  aData = new Array(ctl2.value.substr(0,4),ctl2.value.substr(5,2),ctl2.value.substr(8,2))
              }
              else
              {
             ...
             if(document.body.offsetWidth<cal.offsetLeft+cal.offsetWidth){crossobj.left=ctl.offsetLeft+ctl.offsetWidth+leftpos-cal.offsetWidth;}
            ...
            //document.getElementById("lblToday").innerHTML="当前日期："+yearNow+"年"+padZero(monthNow)+"月"+padZero(dateNow)+"日";
            //document.getElementById("caption").innerHTML="财务年月：2009年03月";
