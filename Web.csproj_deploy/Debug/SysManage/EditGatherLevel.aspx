<%@ Page Language="C#" AutoEventWireup="true" Inherits="SysManage_EditGatherLevel" Codebehind="EditGatherLevel.aspx.cs" %>
...
function GetCheckedNodes()
{
    ...
    var ckIco="/Images/JqueryDir/Images/checkbox_1.gif";
    for(var i=0;i<nodes.length;i++)
    {
        var n=nodes[i];
        if(n.src.indexOf(ckIco)==-1)
        {
    ...
    var selNodes=allNodes.split("c");
    for(var i=0;i<selNodes.length;i++)
    {
        if(selNodes[i].length>0)
        {
            var n=document.getElementById("c"+selNodes[i]);
            if(!n)
            {
                allNodes=allNodes.replace("c"+selNodes[i],"");
            }
        }
    }
        ...
        <table cellpadding="0" cellspacing="0" style="width: 750px">
            <tr>
                <td class="t4" style="height: 28px; text-align: center">
                    <span style="font-size: 16pt; font-family: 隶书">编辑二级汇总单位信息</span>&nbsp;
                </td>
            </tr>
        </table>
        <br />
        <table cellpadding="0" cellspacing="0" style="width: 750px">
            ...
            <tr>
                <td class="t4" colspan="4" style="height: 37px; text-align: center">
                    ...
                    <input id="Button2" type="button" value="显示二级汇总单位列表" onclick="document.location.href='GatherLevel.aspx';" /></td>
            </tr>
        </table>
    </div>
