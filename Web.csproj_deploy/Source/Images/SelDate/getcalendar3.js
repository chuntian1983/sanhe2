    ...
    */
	...
	function init()	{
		if (!ns4)
		{
			...
			if (showToday==1)
			{
				document.getElementById("lblToday").innerHTML =	todayString + " <a onmousemove='window.status=\""+gotoString+"\"' onmouseout='window.status=\"\"' title='"+gotoString+"' style='"+styleAnchor+"' href='javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();'>星期"+dayName[(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)]+"  " + yearNow +  "年" + monthName2[monthNow].substring(0,3) + "月" + dateNow + "日</a>   <a onmousemove='window.status=\"清楚日期选择\"' onmouseout='window.status=\"\"' title='清楚日期选择' style='"+styleAnchor+"' href='javascript:clearCalendar()'>清除</a>"
			}
